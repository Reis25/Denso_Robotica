...
typedef RT_MODEL_projetoRobotica_T RT_MODEL;

/*====================*
 * External functions *
 *====================*/
EXTERN RT_MODEL * projetoRobotica(void);

/* A global buffer for storing error messages (defined in quanser_common library) */
EXTERN char _rt_error_message[512];

/*
   extern void projetoRobotica_initialize();
   extern void projetoRobotica_terminate();
 */
  ...
  _do_assertion(const char * expression, const char * file_name, int line_number)
{
  ...
  rtmSetErrorStatus(projetoRobotica_M, GBLbuf.message);
}
  ...
  control_c_handler(int signal_number)
{
  ...
  {
    pstream_shutdown(projetoRobotica_DW.StreamClient_Stream);
    pstream_shutdown(projetoRobotica_DW.StreamServer_Stream);
  }

  denso_stop_controller(projetoRobotica_DW.DensoRead_Denso)
    ;
}
  ...
  main(int argc, char * argv[])
{
  ...
  if (parseError) {
    (void) fprintf(stderr,
                   "\nUsage: projetoRobotica -option1 val1 -option2 val2 -option3 ...\n\n");
    (void) fprintf(stderr,
  ...
  if (rtExtModeQuarcStartup(rtmGetRTWExtModeInfo(S),
    ...
    if (rtmGetErrorStatus(S) == NULL) {
      ...
      fflush(stdout);
      result = denso_start_controller(projetoRobotica_DW.DensoRead_Denso)
        ;
      if (result == 0) {
